...
func emitRespEvent(event string, payload []byte) []byte {
	return sseEventData(event, payload)
}
...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponsesNonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) []byte {
	...
	if usage := root.Get("usage"); usage.Exists() {
		// Map common tokens
		if usage.Get("prompt_tokens").Exists() || usage.Get("input_tokens").Exists() || usage.Get("completion_tokens").Exists() || usage.Get("output_tokens").Exists() || usage.Get("total_tokens").Exists() {
			inputTokens := usage.Get("prompt_tokens")
			if !inputTokens.Exists() {
				inputTokens = usage.Get("input_tokens")
			}
			resp, _ = sjson.SetBytes(resp, "usage.input_tokens", inputTokens.Int())
			...
			resp, _ = sjson.SetBytes(resp, "usage.output_tokens", outputTokens.Int())
